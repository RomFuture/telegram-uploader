...
def domain_to_source_item_record(
...
def merge_source_item_record(
    existing: SourceItemRecord | None,
    source_item: SourceItem,
) -> SourceItemRecord:
    """Update status fields while keeping folder_id from persistence."""
    folder_id = existing.folder_id if existing is not None else None
    return domain_to_source_item_record(source_item, folder_id=folder_id)
