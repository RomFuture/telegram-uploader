    ...
    def execute(self, source_item_id: UUID) -> None:
        ...
        if not item.source_path.is_file():
            raise FileNotFoundError(
                f"Source file not visible to backup workers: {item.source_path}. "
                "Restart with HOST_SOURCE_MOUNT=/ (default for telegram-uploader) "
                "or place files under your home directory."
            )
