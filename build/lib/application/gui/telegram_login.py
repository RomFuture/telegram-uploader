...
class TelegramSignInDialog(tk.Toplevel):
    ...
    def _send_code(self) -> None:
        ...
        def worker() -> None:
            try:
                phone_code_hash = run_send_login_code(self._config, phone)
            except Exception as exc:
                self.after(0, self._make_send_code_failed_callback(exc))
                return
            self.after(0, self._make_send_code_ok_callback(phone_code_hash))

        threading.Thread(target=worker, daemon=True).start()

    def _make_send_code_ok_callback(self, phone_code_hash: str | None) -> Callable[[], None]:
        return lambda: self._on_send_code_ok(phone_code_hash)
    ...
    def _on_send_code_ok(self, phone_code_hash: str | None) -> None:
    ...
    def _submit_code(self) -> None:
        ...
        def worker() -> None:
            try:
                run_complete_login(self._config, phone, code, self._phone_code_hash)
    ...
    def _on_sign_in_ok(self) -> None:
        self._set_busy(False)
        chat_id = self._settings.target_chat_id.strip()
        group_hint = ""
        if chat_id and not chat_id.startswith("-100"):
            group_hint = (
                "\n\nNote: Target chat ID looks like a user id, not a group.\n"
                "On General tab use the backup group id (usually -100…).\n"
                "Test Client API will fail until that is fixed."
            )
        messagebox.showinfo(
            "Signed in",
            f"Session saved to:\n{self._config.session_path}\n\n"
            f"Config: {self._env_path}\n\n"
            "Telegram login is complete. Fix Target chat ID if needed, "
            "then click Test Client API."
            f"{group_hint}",
            parent=self,
        )
        self.destroy()
