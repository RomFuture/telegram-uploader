...
| Client API: sign-in + upload + Test Client API | ✅ GUI / `telegram-uploader-login` |
| Restore download (Client API) | 🟡 smoke TBD → [TELEGRAM_CLIENT_API_MIGRATION.md](TELEGRAM_CLIENT_API_MIGRATION.md) |
...
- Restore download на Client API — smoke TBD ([migration](TELEGRAM_CLIENT_API_MIGRATION.md))
...
**Сейчас:** Unlock → file explorer (sidebar + Name/Folder/Size/Modified/Status) → progress drawer внизу. English-only. Данные через `BackendReceiver` → `BackupApi`.
