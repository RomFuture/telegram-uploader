...
- Слой/подзадача сделана → тесты зелёные → **ручной smoke пользователем** → пункт удаляется отсюда.
- Большие планы с деталями — отдельные `docs/*_MIGRATION.md`; в бэклоге только ссылка + gate.
